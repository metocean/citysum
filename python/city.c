...
#undef PERMUTE3
#define PERMUTE3(a, b, c) do { swap_uint32(a, b); swap_uint32(a, c); } while (0)
...
static inline uint128 make_pair(uint64 first, uint64 second) {
  uint128 r;
  r.first = first;
  r.second = second;
  return r;
...
static uint128 WeakHashLen32WithSeedsNums(
...
static uint128 WeakHashLen32WithSeeds(
  ...
  return WeakHashLen32WithSeedsNums(Fetch64(s),
                                    Fetch64(s + 8),
                                    Fetch64(s + 16),
                                    Fetch64(s + 24),
                                    a,
...
static uint128 CityMurmur(const char *s, size_t len, uint128 seed) {
  ...
  a = HashLen16(a, c);
  b = HashLen16(d, b);
  return make_pair(a ^ b, HashLen16(b, a));
}
...
uint128 CityHash128WithSeed(const char *s, size_t len, uint128 seed) {
  ...
  return make_pair(HashLen16(x + v.second, w.second) + y,
...
uint128 CityHash128(const char *s, size_t len) {
  return len >= 16 ?
      CityHash128WithSeed(s + 16, len - 16,
                          make_pair(Fetch64(s), Fetch64(s + 8) + k0)) :
      CityHash128WithSeed(s, len, make_pair(k0, k1));
}
